          ...
          <tr>
            ...
            <td>
              <div class="table-actions">
                <a href="<?= route_to('employees.locations.edit', $l['id']) ?>" class="btn btn-tertiary btn-sm" aria-label="Editar <?= esc($l['name']) ?>">
                ...
                <form action="<?= route_to('employees.locations.destroy', $l['id']) ?>" method="post"
                      onsubmit="return confirm('¿Eliminar la ubicación «<?= esc($l['name']) ?>»?')">
                  <?= csrf_field() ?>
                  <button type="submit" class="btn btn-tertiary btn-sm" style="color:var(--color-critical-default);" aria-label="Eliminar <?= esc($l['name']) ?>">
