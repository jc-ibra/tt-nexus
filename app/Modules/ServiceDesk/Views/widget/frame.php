...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .msg{max-width:85%;padding:9px 12px;border-radius:var(--radius);white-space:pre-wrap;word-wrap:break-word}
  ...
  .card{align-self:stretch;background:var(--card);border:1px solid var(--line);border-radius:var(--radius);padding:12px}
...
<?php else: ?>
  ...
  (function () {
    ...
    function submit(message) {
      ...
      post(CFG.chatUrl, { history: history, message: message }).then(function (res) {
    ...
    function createTicket() {
      ...
      setBusy(true); typing(true);
      post(CFG.ticketUrl, { draft: draft }).then(function (res) {
        typing(false); setBusy(false);
        if (!res.ok) {
    ...
    if (CFG.welcome) { addMsg("bot", CFG.welcome); }
    text.focus();
  })();
  </script>
<?php endif; ?>
